  ...
  return (
    <div className="bg-gradient-to-b from-bgGradientStart to-white md:pt-4">
      <div className="max-w-[640px] px-5 md:max-w-[900px] mx-auto">
        {data?.blogPost?.title && (
          <h1 className="logoFont text-3xl py-4 text-gray-600 font-semibold md:text-5xl">
            {data?.blogPost?.title}
          </h1>
        )}
          ...
          <div>
            <div className="flex items-center justify-start space-x-3 pb-3">
              ...
              <div className="flex items-start flex-col justify-start">
                {data?.blogPost?.authorName && (
                  <>
                    <p className="text-gray-700">
                      By {data?.blogPost?.authorName}
                    </p>
                  </>
                )}
                {data?.blogPost?.publishedDate && (
                  <p className="text-xs text-gray-500">
                    Published {data?.blogPost?.publishedDate}
                  </p>
                )}
              </div>
            </div>
          </div>
          {data?.blogPost?.editedBy && (
            <div className="flex items-center justify-start space-x-1">
              <GrCheckmark />
              <p className="text-xs text-gray-500">Edited by</p>
              <p className="text-gray-700">{data?.blogPost?.editedBy}</p>
            </div>
          )}
        </div>
